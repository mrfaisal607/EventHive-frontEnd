...
import { Eye, EyeOff, Briefcase, Loader } from "lucide-react"; // Added Loader for loading state
...
const RegisterVendor = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    // 🔹 Validate Business Name (Minimum 3 Characters)
    if (formData.businessName.length < 3) {
      setFormError("⚠️ Business name must be at least 3 characters!");
      return;
    }
    ...
    try {
      const res = await dispatch(
        registerUser({
          ...
          role: "vendor", // ✅ Ensure correct role
        })
      );
